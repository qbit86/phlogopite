...
namespace Phlogopite
{
    internal static class FormattingHelpers
    {
            ...
            ReadOnlySpan<NamedProperty> writerProperties)
        {
            ...
            int textLength = 1 + (text?.Length ?? 0); // “ Hello, world!”
            ...
            for (int i = 0; i != writerProperties.Length; ++i)
                capacity += writerProperties[i].AsString?.Length ?? 0;

            for (int i = 0; i != userProperties.Length; ++i)
                capacity += (userProperties[i].Name?.Length ?? 0) + userProperties[i].AsString?.Length ?? 16;
